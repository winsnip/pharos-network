...
export async function withdraw({
     ...
     const usdtBalance = await accountUSDTBalance({ signer, provider, proxyUrl: PROXY_URL })
     ...
     const amount = parseUnits(availableBalanceSize.split(".")[0], Number(decimalsToken)) * BigInt(amountInPercent) / 100n
     console.log(`Withdrawing ${formatUnits(amount, Number(decimalsToken))} ${coinName}`)
...
export async function accountUSDTBalance({
     ...
     const url = "https://api.bitverse.zone/bitverse/trade-data/v1/account/balance/allCoinBalance"
     ...
     const { availableBalanceSize, contractAddress, coinName } = json.result.coinBalance[0]
