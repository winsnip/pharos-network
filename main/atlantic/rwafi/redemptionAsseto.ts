...
import { redemption } from "@scripts/pharosNetwork/atlantic/rwafi/asseto";
...
async function main() {
  ...
  const tokenIn: string = assetoAddresses.filter(
    (address) => address.name == "CASH+"
  )[0].contract;
  const tokenOut: string = assetoAddresses.filter(
  ...
  for(let index = 1; index <= env.LOOP_COUNT ; index++){
    console.log(`Task asseto redemption ${index}/${env.LOOP_COUNT}`);
